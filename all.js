...
axios.get("https://raw.githubusercontent.com/hexschool/js-training/main/travelApi.json").then((response)=>{
    ...
    renderDonutChart(getRecentRegion());
});
...
// ************** 方法：遍歷 data array 的每一個 object，用if-else把現有的地區屬性存到 regionObject
// ************** 再使用Object.keys 和 Object.values，整理出陣列資料，並用一個遍歷整理出 C3 donut 需要的資料格式
function getRecentRegion () {
    let regionObject = {};
    data.forEach(function (item){
        if(regionObject[item.area] == undefined){
            regionObject[item.area] = 1;
        }else{
            regionObject[item.area] += 1;
        }
    });
    let keysArray = Object.keys(regionObject);
    let valuesArray = Object.values(regionObject);
    let chartDataArray = [];
    keysArray.forEach(function (item, index) {
        let arr = [item, valuesArray[index]];
...
// 更新資料之後，讓 C3 重新載入資料
function renderDonutChart (chartDataArr) {
    chart.load({columns: chartDataArr});
}
